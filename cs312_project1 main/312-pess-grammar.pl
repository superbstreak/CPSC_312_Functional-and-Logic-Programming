...
% read_sent_helper([]) :- peek_char(Ch), Ch = '%', write('Comment Detected'),nl, get_char(Ch).
...
% comment_detector(I,O) :- peek_char(I), Ch = '%', !, get_char(I), write(I).		
...
%%%%%%%%%%%%%%%%%%% grammar for parsing vocab FWT Q3 %%%%%%%%%%%%%%%%%%%
% Parser Example
% late adjective and
% project noun and
% silly adverb and
...
% vocab parser
vocab       --> wp, \+ [_].
vocab       --> wp, delimiter, vocab.

% the format Word connector PoS <- then assert Pos
wp --> [Word], connector, [noun],      { assert(n(Word)) }.
wp --> [Word], connector, [verb],      { assert(v(Word)) }.
wp --> [Word], connector, [adjective], { assert(adj(Word)) }.
wp --> [Word], connector, [adverb],    { assert(adv(Word)) }.

% separation between each sentence
delimiter   --> [].
delimiter   --> ['and'].

% connectors bwtween words and their part of speech
% Ex: late adjective
% Ex: word is a noun
% Ex: last is an adjective
connector   --> [].
connector   --> ['is'].
connector   --> ['is'], ['a'].
connector   --> ['is'], ['an'].
...
% 'what is it' and 'what does it have' are special 
% cases that don't fit in with any other template
goal(Terms) --> [what], is_it, { Terms = [attr(is_a, _, _)], assertz(rule(top_goal(Terms), Terms)) }.
goal(Terms) --> [what], does_it, [have], { Terms = [attr(has_a, _, _)], assertz(rule(top_goal(Terms), Terms)) }.

% what does it VERB_PHRASE
goal(Terms) --> [what], does_it, vp([attr(does,X,_)]), { Terms = [attr(does,X,_)], assertz(rule(top_goal(Terms), Terms)) }.

% yes/no questions
goal(Terms) --> question(Terms), { assertz(rule(top_goal(yes), Terms)) }.

% the question form
question(NPTerms) --> is_it, np(NPTerms).   % is it NOUN_PHRASE
question(VDTerms) --> does_it, vp(VDTerms). % does it VERB_PHRASE
...
% ===================================================================
% Main Project #6
% Use WordNet and ProNTo_Morph to expand vocab!
...
% the word is morphed into forms
% Each word has
% s(Synset_ID, Word Number, Actual Word, synset type, sense number, Tag).
% (Synset_ID, Word Number [Serially], Actual Word, synset type,
% sense number [how common the word is], Tag [how common a word is in
% relation to a text]).
%
% Example 1 from morph_atoms_bag
% The word 'triples' returns the following:
% [[[[triples]],[[tripl,-pl]],[[triplis,-pl]],[[triple,-s]]]]
%
% Example 2 from morph_atoms_bag
% The word 'says' returns the following:
% [[[[says]],[[say,-s]]]]
%
% Generic
% [[...]] <- outter format
% [[ [[words]], [[word, -s]] ...]] <- Inner fromat
% 
% the letter 'a' is retracted because it is causing problem

wordType(Word, Type) :- morph_atoms_bag(Word, [[[Word|_]]|_]), s(_SID, _, Word, Type, _, _), retract(n(a)).
...
word_line_morphs :- write('|: '), 
	read_input(Word),
	morph_atoms_bag(Word,Result),
	print(Result).
% ================================================
% Helper read_input/1 
% read in the user input. Modified to automatically
% stop reading after enter is pressed (no period
% required as given in the sample output in the
% project description) Update: This modification
% has been disabled due to description update 28th
% 
% Source: 312-pess-grammar.pl By Steve Wolfman
% ================================================

% Read in the user input
read_input(_) :- peek_char(Ch), Ch = 'end_of_file', !, fail.
read_input(S) :- read_input_helper(S).

% Read each individual words

% Stop at end of file
read_input_helper([]) :- peek_char(Ch),Ch = 'end_of_file', !, fail.

% Stop at end of line (no longer necessary in the updated project description)
% read_input_helper([]) :- peek_char(Ch), char_type(Ch, end_of_line), !, get_char(_).

% Stop at a period.
read_input_helper([]) :- peek_char(Ch),Ch = '.', !, get_char(Ch).

% Eat whitespace
read_input_helper(Words) :- peek_char(Ch),char_type(Ch, space), !, get_char(Ch),read_input_helper(Words).

% Read quoted words
read_input_helper([Word|Words]) :-
        peek_char(Ch), Ch = '"', !,
        read_word_to(ChWord), 
        atom_chars(Word, ChWord), 
        read_input_helper(Words).
...
% Read a word delimited by whitespace or a period (which ends the sentence).
% End of line is no longer necessary in the updated project description
...
adj(spheroidal).
adj(spherical).
adj(Word) :- wordType(Word, a),!. 
adj(Word) :- wordType(Word, s),!. % sat type in wordnet
